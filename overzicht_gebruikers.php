<?php
    ...
    if(isset($_GET['id'])) {
        $db->update_or_delete("DELETE FROM gebruikers WHERE id=:id", ['id'=>$_GET['id']], "overzicht_gebruikers.php");
            $loginError = $db->update_or_delete($sql, $placeholder, "overzicht_gebruikers.php");
            var_dump($loginError);
    }

    if(isset($_POST['export'])){
        $filename = "gebruikers_data_export.xls";
        header("Content-Type: application/vnd.ms-excel");
        header("Content-Disposition: attachment; filename=\"$filename\"");
        $print_header = false;
        
        $result = $db->select("SELECT gebruikers.id, usertype.type, gebruikers.voornaam, gebruikers.gebruikersnaam, gebruikers.wachtwoord, gebruikers.email, gebruikers.telefoonnummer, gebruikers.created_at, gebruikers.updated_at 
        FROM gebruikers
            INNER JOIN usertype ON gebruikers.usertype_id = usertype.id", []);
...
<body>
    ...
    $result_set = $db->select("SELECT gebruikers.id, usertype.type, gebruikers.voornaam, gebruikers.gebruikersnaam, gebruikers.wachtwoord, gebruikers.email, gebruikers.telefoonnummer, gebruikers.created_at, gebruikers.updated_at 
    FROM gebruikers, usertype", []);
    $columns = array_keys($result_set[0]);

    $result_set1 = $db->select("SELECT gebruikers.id, usertype.type, gebruikers.voornaam, gebruikers.gebruikersnaam, gebruikers.wachtwoord, gebruikers.email, gebruikers.telefoonnummer, gebruikers.created_at, gebruikers.updated_at 
    FROM gebruikers
        INNER JOIN usertype ON gebruikers.usertype_id = usertype.id", []);
    ?>
...
?>
            <tr>
                ...
                <?php } ?>
                <td>
                    <a href="edit_gebruiker.php?id=<?php echo $row_id; ?>" class="settings" ><i class="material-icons">&#xE8B8;</i></a>
                    <a onclick="return confirm('Are you sure you want to delete this entry?')"
                        href="overzicht_gebruikers.php?id=<?php echo $row_id; ?>"
