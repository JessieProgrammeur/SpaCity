<?php
    ...
    if(isset($_GET['id'])) {
        $db->update_or_delete("DELETE FROM usertype WHERE id=:id", ['id'=>$_GET['id']], "overzicht_usertype.php");
            $loginError = $db->update_or_delete($sql, $placeholder, "overzicht_usertype.php");
            var_dump($loginError);
    }

    if(isset($_POST['export'])){
        $filename = "usertypes_data_export.xls";
        header("Content-Type: application/vnd.ms-excel");
        header("Content-Disposition: attachment; filename=\"$filename\"");
        $print_header = false;
        
        $result = $db->select("SELECT * FROM usertype", []);
...
<body>
    ...
    $result_set = $db->select("SELECT * FROM usertype", []);
    $columns = array_keys($result_set[0]);

    $result_set1 = $db->select("SELECT * FROM usertype", []);
    ?>
    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            ...
            <tr>
                ...
                <?php } ?>
                <td>
                    <a href="edit_usertype.php?id=<?php echo $row_id; ?>" class="settings" ><i class="material-icons">&#xE8B8;</i></a>
                    <a onclick="return confirm('Are you sure you want to delete this entry?')"
                        href="overzicht_usertype.php?id=<?php echo $row_id; ?>"
