<?php
    ...
    if(isset($_GET['id'])) {
        $db->update_or_delete("DELETE FROM betalingen WHERE id=:id", ['id'=>$_GET['id']], "overzicht_betalingen.php");
            $loginError = $db->update_or_delete($sql, $placeholder, "overzicht_betalingen.php");
            var_dump($loginError);
    }

    if(isset($_POST['export'])){
        $filename = "betalingen_data_export.xls";
        header("Content-Type: application/vnd.ms-excel");
        header("Content-Disposition: attachment; filename=\"$filename\"");
        $print_header = false;
        
        $result = $db->select("SELECT betalingen.id, betalingen.status, betalingen.created_at, betalingen.updated_at 
        FROM betalingen", []);
...
<body>
    ...
    $result_set = $db->select("SELECT betalingen.id, betalingen.status, betalingen.created_at, betalingen.updated_at 
    FROM betalingen", []);
    $columns = array_keys($result_set[0]);

    $result_set1 = $db->select("SELECT betalingen.id, betalingen.status, betalingen.created_at, betalingen.updated_at 
    FROM betalingen", []);
    ?>
    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-5">
                        <h2>Status Betalingen <b>Management</b></h2>
                    </div>
                    <div class="col-sm-7">
                        <a href="voeg_betaling_toe.php" class="btn btn-secondary"><i class="material-icons">&#xE147;</i> <span>Voeg Betaling Toe</span></a>
                        
                        <form method="post" action="overzicht_betalingen.php" class="row">
...
?>
            <tr>
                ...
                <?php } ?>
                <td>
                    <a href="edit_betaling.php?id=<?php echo $row_id; ?>" class="settings" ><i class="material-icons">&#xE8B8;</i></a>
                    <a onclick="return confirm('Are you sure you want to delete this entry?')"
                        href="overzicht_betalingen.php?id=<?php echo $row_id; ?>"
